...
export default function PodcastsGrid({ query, items }: GridProps) {
  ...
  const filteredItems = useMemo(() => {
    ...
    return sourceItems
      .filter((item) => {
        // Filter for podcasts - check format field or tags containing 'podcast'
        const isPodcastFormat = item.format === 'Podcast';
        const hasPodcastTag = item.tags?.some(tag => 
          tag.toLowerCase().includes('podcast')
        );
        return isPodcastFormat || hasPodcastTag;
      })
      .filter((item) => {
  ...
  if (filteredItems.length === 0) {
    return (
      <section className="space-y-3">
        ...
        <div className="text-center py-12 text-gray-500">No podcasts available</div>
      </section>
    );
  }
