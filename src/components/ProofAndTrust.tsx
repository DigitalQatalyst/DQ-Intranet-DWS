...
const testimonials: Testimonial[] = [{
  ...
  companyLogo: 'https://image2url.com/images/1760524231537-47b810dd-94eb-4571-a6a9-0a9c6fbfb390.jpg',
      ...
      <div ref={modalRef} className="bg-white rounded-2xl overflow-hidden max-w-4xl w-full max-h-[90vh] shadow-2xl transform transition-all duration-300 animate-fadeIn">
        ...
        <div className="p-6">
          <div className="flex items-center mb-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900">How {testimonial.company} scaled with Enterprise Journey</h3>
              <div className="flex items-center mt-1">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={16} className={i < testimonial.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start mb-6">
            ...
            <div>
              <p className="text-gray-700 italic mb-3">"{testimonial.fullQuote}"</p>
              <div>
...
const VideoTestimonialCarousel = () => {
  ...
  const openModal = (testimonial: Testimonial) => { setSelectedTestimonial(testimonial); setIsModalOpen(true); };
    ...
    <div className="relative">
      ...
      >
        {testimonials.map((testimonial, index) => (
          <div key={testimonial.id} className="min-w-full sm:min-w-[calc(100%/2-12px)] lg:min-w-[calc(100%/3-16px)] flex-shrink-0 snap-center">
      ...
      <div className="flex justify-center mt-4 gap-2">
        {testimonials.map((_, index) => (
          <button
            key={index}
            className={`w-2 h-2 rounded-full transition-all ${activeIndex === index ? 'bg-dq-coral w-6' : 'bg-gray-300'}`}
...
const PartnerCategoryCard = ({ category }) => {
  ...
  return (
    <div
      ref={ref}
      className={`relative overflow-hidden rounded-xl p-6 transition-all duration-500 ease-out transform ${isHovered ? 'shadow-md scale-[1.02]' : 'shadow-sm'}`}
      style={{ background: isHovered ? `linear-gradient(to bottom right, #f9fafb, #ffffff)` : '#ffffff' }}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div
        className={`w-14 h-14 rounded-lg flex items-center justify-center mb-4 transition-all duration-300 ${isHovered ? 'scale-110' : ''}`}
        style={{ backgroundColor: `rgba(var(--color-${category.color}-rgb), 0.1)`, color: `var(--color-${category.color})` }}
      >
...
const FeaturedPartnersCarousel = () => {
    ...
    const interval = setInterval(() => {
      if (carouselRef.current) {
        ...
        if (currentScroll >= maxScroll - 10) {
          ...
          carouselRef.current.scrollTo({ left: currentScroll + 200, behavior: 'smooth' });
        }
      }
    }, 5000);
    return () => clearInterval(interval);
  ...
  const handlePrev = () => { if (carouselRef.current) carouselRef.current.scrollTo({ left: carouselRef.current.scrollLeft - 300, behavior: 'smooth' }); };
      ...
      <div className="container mx-auto px-4">
          ...
          <FadeInUpOnScroll className="text-center mb-10">
            ...
            <div>
              <p className="text-lg text-gray-600 max-w-3xl mx-auto mb-8 text-balance clamp-2">
                Agile working empowers teams to adapt, collaborate, and grow faster together.
              </p>
            </div>
          </FadeInUpOnScroll>
        ...
        <div className="mb-16 bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 md:p-12 overflow-hidden relative">
          <FadeInUpOnScroll className="text-center mb-10 relative z-10">
            <h2 className="text-3xl font-bold text-gray-900 mb-3 clamp-1">Success Stories from DQ Employees</h2>
            <div>
              <p className="text-lg text-gray-600 max-w-3xl mx-auto clamp-2">Discover how DQ teams work smarter and collaborate better every day.</p>
            </div>
          </FadeInUpOnScroll>
        ...
        <div className="mb-16 bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 md:p-12 overflow-hidden relative">
          <HorizontalScrollReveal direction="left" className="text-center mb-10 relative z-10">
            <h2 className="text-3xl font-bold text-gray-900 mb-3 clamp-1">Our Four Pillars of Success</h2>
            <div className="relative">
              <p className="text-lg text-gray-600 max-w-3xl mx-auto clamp-2">Governance, Operations, Platforms, and Delivery — the four pillars driving DQ’s success.</p>
            </div>
